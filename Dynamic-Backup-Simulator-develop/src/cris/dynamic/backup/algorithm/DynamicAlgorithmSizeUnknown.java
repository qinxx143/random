...
public class DynamicAlgorithmSizeUnknown extends Scheduler {
    ...
    private final Map<String, Map<String, ArrayList<HistoricalDataPoint>>> storageAssociationData;    //Map<BackupName, Map<StorageName, ArrayList<Throughputs>
    private final Map<String, ArrayList<HistoricalDataPoint>>              backupHistoricalData;      //<String(backupName), ArrayList<Throughputs>(throughput values for that backup)>

    public DynamicAlgorithmSizeUnknown() {
        day = 0;
        storageAssociationData = new HashMap<String, Map<String, ArrayList<HistoricalDataPoint>>>();
        backupHistoricalData = new HashMap<String, ArrayList<HistoricalDataPoint>>();
    }

    @Override
    public String getName() {
        return "DynamicAlgorithmSizeUnknown";
    }
    ...
    public Map<String, String> getNewBackups(final long currentTime) {
        ...
        Collections.sort(backupList, new Comparator<Backup>() { //sort by priority
            ...
            private double calculateBackupPriority(final Backup backup) {
                if (backupHistoricalData.get(backup.getName()).isEmpty()) {
                    //No historical data, priority set to 0.
                    return 0;
                }

                //There is historical data, use that to determine priority
                final double historicalThroughput = calculateExpectedThroughput(backupHistoricalData.get(backup.getName()));
                final double historicalSize = calculateExpectedSize(backupHistoricalData.get(backup.getName()));

                if (backup.getConstraint().getEndConstraint() == -1) { //no end constraints determined LongestBackupFirst mode
                    return 1 / (historicalSize / historicalThroughput); //longest backup first

                } else { //We have an end constraint and we use it to determine who is added first
                    return ((currentTime - backup.getConstraint().getEndConstraint()) - (historicalSize / historicalThroughput))
                            / (historicalSize / historicalThroughput);
        ...
        Collections.sort(stuList, new Comparator<StorageDevice>() {
            ...
            private double calculateStoragePriority(final StorageDevice stu) {
                ...
                for (int i = 0; i < activeBackups.size(); i++) {
                    final Backup backup = DynamicAlgorithmSizeUnknown.super.getBackups().get(activeBackups.get(i));
                    if (!backupHistoricalData.get(backup.getName()).isEmpty()) {
                        //If there is historical data estimate how much time is left.
                        priorityToReturn += backup.getDataLeft() / calculateExpectedThroughput(backupHistoricalData.get(backup.getName()));
                    } else {
    ...
    public void notateHistoricalData(final Backup completedBackup) {
        final ArrayList<HistoricalDataPoint> historicalValues = storageAssociationData.get(completedBackup.getName()).get(completedBackup.getStorageName());
        if (historicalValues == null) {
            storageAssociationData.get(completedBackup.getName()).put(completedBackup.getStorageName(), new ArrayList<HistoricalDataPoint>());
        }

        storageAssociationData.get(completedBackup.getName()).get(completedBackup.getStorageName())
        ...
        backupHistoricalData.get(completedBackup.getName())
        .add(new HistoricalDataPoint(day, day + historicalDataExpirationTime, completedBackup.getOverallThroughput(), completedBackup.getDataSize()));
    ...
    public void removeOldHistoricalData(final int day) {
        ...
        for (final Map<String, ArrayList<HistoricalDataPoint>> map : storageAssociationData.values()) {
        ...
        for (final ArrayList<HistoricalDataPoint> list : backupHistoricalData.values()) {
            for (final Iterator<HistoricalDataPoint> iter = list.iterator(); iter.hasNext();) {
                final HistoricalDataPoint point = iter.next();
                if (point.isExpired(day)) {
                    iter.remove();
    ...
    public void setBackups(Map<String, Backup> backups) {
        ...
        if (day == 0) {
            for (final String backupName : backups.keySet()) {
                storageAssociationData.put(backupName, new HashMap<String, ArrayList<HistoricalDataPoint>>());
                backupHistoricalData.put(backupName, new ArrayList<HistoricalDataPoint>());
            }
        }
    }
     ...
     * @return average
     */
    private double calculateExpectedSize(List<HistoricalDataPoint> dataPoints) {
        double sum = 0;
        double weightSum = 0;
        if (dataPoints != null && !dataPoints.isEmpty()) {
            for (final HistoricalDataPoint dataPoint : dataPoints) {
                sum += dataPoint.getWeightedSize(day);
                weightSum += dataPoint.getWeight(day);
            }
            return sum / weightSum;
        }
        return 1; //This is an important 1. It makes it so that if no data is present, backups are assumed to have length 1.
    }
    ...
    private double calculateExpectedThroughput(List<HistoricalDataPoint> dataPoints) {
        ...
        return 1; //This is an important 1. It makes it so that if no data is present, backups are assumed to have a throughput of 1 MB/s
    }

    private int getStorageRanking(final String backupName, final String storageName) {
        int higher = 0;
        final double expectedThroughput = calculateExpectedThroughput(storageAssociationData.get(backupName).get(storageName));
        for (final Map.Entry<String, ArrayList<HistoricalDataPoint>> mapEntry : storageAssociationData.get(backupName).entrySet()) {
